 ...
 */
class DepthAndEdgeFilterEvaluator implements Evaluator {
    private final Evaluator depthEvaluator;
    private final Predicate<Path> edgeReturnFilter;


    DepthAndEdgeFilterEvaluator(Collection<FilterDescription> filterDescriptions, boolean restrictMode,
            boolean matchCase, int maxDepth) {
        depthEvaluator = Evaluators.toDepth(maxDepth);

        edgeReturnFilter = new EdgeReturnFilter(filterDescriptions, restrictMode, matchCase);
    }
    ...
    public Evaluation evaluate(Path path) {
        ...
        boolean returnEdge = edgeReturnFilter.accept(path);
