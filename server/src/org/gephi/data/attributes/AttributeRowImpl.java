...
public class AttributeRowImpl implements AttributeRow {
    ...
    private void setValue(int index, AttributeValueImpl value, boolean doUpdateColumns) {
        ...
        if (!(oldValue != null && oldValue.equals(value)) && index > 0 && !value.getColumn().getOrigin().equals(AttributeOrigin.COMPUTED)) {    //0 is the index of node id and edge id cols, not useful to send these events
            if (oldValue != null) {
